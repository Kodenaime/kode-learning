...
const Enrollments = () => {
  ...
  const {enrolledCourses, fetchEnrolledCourses, calculateTime, navigate} = useContext(AppContext)
                ...
                <tr key={index} className='border-b border-gray-500/20'>
                  <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                    ...
                    <div className='flex-1'>
                      ...
                      <Line strokeWidth={2}  className='bg-gray-300 rounded-full' 
                  ...
                  <td className='px-4 py-3 max-sm:text-right'>
                    <button
                      ...
                      className={`px-3 sm:px-5 py-1.5 sm:py-2 max-sm:text-xs text-white rounded cursor-pointer ${
                        progressArray[index] && progressArray[index].lectureCompleted 
                        / progressArray[index].totalLectures === 1  ? 'bg-orange-500' : 'bg-blue-600'                         
                      }`} >
                            {
                            progressArray[index] &&
